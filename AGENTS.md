...
- **Screen-time tracking** (`src/main/screen-time.ts`): per-profile used-seconds ledger (`<userData>/screen-time-<profileId>.json`, resets daily); ticker emits status every second (`ScreenTimeStatus { usedSec, inUsageWindow }`) driving usage-hours enforcement; the toolbar panel shows the day's used time.
...
- **Activity log** gains `restricted`, `wifi-connect` kinds; admin Usage tab shows per-day per-profile usage.
